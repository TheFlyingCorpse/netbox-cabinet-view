...
- ~~**Full-size view + port overlay for rack-mount devices without mounts.**~~ **shipped in v0.7.3** via option (a): the Layout tab now appears for any device whose profile defines a `port_map`, even without `hosts_mounts` or a Mount, and renders a full-size front-panel SVG with the interactive port overlay (`svg/front_panel.py` + the `cabinet-layout/front-panel.svg` endpoint).
