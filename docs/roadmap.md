...
- **Full-size view + port overlay for rack-mount devices without mounts.** Standalone rack-mount devices (e.g. a 1U switch with no internal cabinet structure) show their `DeviceType.front_image` line-art in the rack elevation thumbnail, but there's no Layout tab to view them full-size with the interactive port overlay (clickable pins, status colours, drag-to-place). The Layout tab only appears for devices with `hosts_mounts=True` + at least one Mount. Options: (a) show a read-only "front panel" tab for any device with a `DeviceMountProfile.port_map`, even without mounts, (b) render port overlay directly on the rack elevation for these devices, (c) auto-create a dummy "front panel" mount so the existing Layout tab works.
