...
- **Port overlay on rack-mount devices without mounts.** Currently the port overlay and plugin line-art only render for devices that have `hosts_mounts=True` with at least one Mount (the Layout tab). Standalone rack-mount devices (e.g. a 1U switch with no internal cabinet structure) show as plain coloured rectangles in the rack elevation and have no Layout tab. The front_image from `DeviceMountProfile` is a plugin-level fallback that core NetBox's rack elevation doesn't know about. Options: (a) teach the rack elevation patch to also handle non-host devices that have a `DeviceMountProfile.front_image`, (b) auto-copy the plugin front_image to the core `DeviceType.front_image` field, or (c) render port overlay directly on the rack elevation for these devices.
